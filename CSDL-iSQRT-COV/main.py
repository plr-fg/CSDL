...
def main():
    ...
    criterion = PseudoLabelCrossEntropyLoss(n_classes=args.num_classes, feature_dim=32896,
                                            epsilon=0.3, lamda=0.003, similarity='cosine').cuda(args.gpu)

    center_param_optimizer = torch.optim.SGD(criterion.parameters(), lr=0.5)
    w_pseudo_label_plan = [0.2] * args.epochs
    # w_pseudo_label_plan[:10] = list(np.linspace(0.5, 0.1, 10))
    ...
    mean_feature = get_init_center(model, train_loader, args.num_classes, n_dim_feature=32896)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        trainObj, top1, top5 = train(train_loader, model, criterion, optimizer, center_param_optimizer, epoch, w_pseudo_label_plan)
        # evaluate on validation set
        valObj, prec1, prec5 = validate(val_loader, model, ce_criterion)
        # update stats
    ...
    if evaluate_transforms is not None:
        ...
        validate(evaluate_loader, model, ce_criterion)




def train(train_loader, model, criterion, optimizer, center_param_optimizer, epoch, w_pseudo_label_plan):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        loss = criterion(embedding, output, target, w=w_pseudo_label_plan[epoch])
        ...
        optimizer.zero_grad()
        center_param_optimizer.zero_grad()
        loss.backward()
        optimizer.step()

        for param in criterion.parameters():
                param.grad.data *= 0.5
        center_param_optimizer.step()
...
def validate(val_loader, model, ce_criterion):
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    top5 = AverageMeter()
    ...
    with torch.no_grad():
        ...
        for i, (input, target) in enumerate(val_loader):
            ...
            if len(input.size()) > 4:# 5-D tensor
                ...
                _, output = model(input.view(-1, ch, h, w))
